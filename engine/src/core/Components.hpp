...
// Primitive" menu): Torus appended the same way, for the same reason
// (=7) -- see Mesh::createTorus()'s own comment for why it gets a real
// MeshSourceKind, unlike Mesh::createCylinder()/createWedge() (used by
// BlockBuilderPlugin's palette), which deliberately don't -- see
// createCylinder()'s own comment on that accepted, documented
// "live-session only" limitation. The menu's own Sphere entry needs no
// new value here at all -- it reuses BlockBuilderPlugin::spawnBlock()'s
// own existing "zero-half-height Capsule" convention (MeshSourceKind::
// Capsule, halfHeight param = 0), already a real, fully round-tripping
// sphere.
enum class MeshSourceKind { Box, Plane, Capsule, Quad, Obj, Gltf, Fbx, Torus };
...
// halfHeight=y (a real sphere is a Capsule with halfHeight=0 -- see
// BlockBuilderPlugin::spawnBlock()'s own comment), Quad: halfSize=x,
// Torus: majorRadius=x/minorRadius=y) and is unused when kind == Obj,
// Gltf, or Fbx, where `path` (a real filesystem path to the source
// .obj/.gltf/.glb/.fbx) is used instead.
